        ...
        it "within-DB swap equals a re-solve of the relocated matrix" $ do
            ...
            case pickGlobalAB db of
                ...
                Just (aPid, bPid) -> do
                    ...
                    xRef <- solveWithSharedSolver refSolver =<< either (fail . show) pure (buildDemandVector db 0)
                    case gRes of
        ...
        it "cross-DB swap removes 'from' from the root and emits one link per consumer" $ do
            ...
            case pickGlobalAB root of
                ...
                Just (aPid, _) -> do
                    ...
                    xRef <- solveWithSharedSolver refSolver =<< either (fail . show) pure (buildDemandVector root 0)
                    case gRes of
                        Right (xGlobal, links) -> do
