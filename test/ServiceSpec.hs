...
import Service ( validateUUID, parseProcessIdFromText, validateProcessIdInMatrixIndex
               ...
               , isResourceExtraction, extractCompartment
               , filterTreeExport, buildUnitGroups
               )
import API.Types
    ( TreeExport(..), TreeMetadata(..), ExportNode(..), TreeEdge(..)
    , NodeType(..), EdgeType(..), FlowInfo(..)
    )
...
mkBioFlow :: Text -> Flow
mkBioFlow cat = Flow
    { flowId = nil, flowName = "test", flowCategory = cat
    , flowSubcompartment = Nothing, flowUnitId = nil
...
mkTreeExport nodeSpecs edgeSpecs =
    let mkNode (nid, parent, name) = (nid, ExportNode
            { enId = nid, enName = name, enDescription = []
            , enLocation = "", enUnit = "kg"
            , enNodeType = ActivityNode, enDepth = 0
