...
minimalFlow fid name = Flow
    { flowId = fid, flowName = name, flowCategory = ""
    , flowSubcompartment = Nothing, flowUnitId = UUID.nil
...
minimalActivity name loc exs = Activity
    { activityName = name, activityDescription = []
    , activitySynonyms = M.empty, activityClassification = M.empty
    , activityLocation = loc, activityUnit = "kg"
    , exchanges = exs, activityParams = M.empty, activityParamExprs = M.empty }

refExchange :: UUID.UUID -> Exchange
refExchange fid = TechnosphereExchange
    { techFlowId = fid, techAmount = 1.0, techUnitId = UUID.nil
    , techIsInput = False, techIsReference = True
    , techActivityLinkId = UUID.nil, techProcessLinkId = Nothing, techLocation = "GLO" }

inputExchange :: UUID.UUID -> Text -> Exchange
inputExchange fid loc = TechnosphereExchange
    { techFlowId = fid, techAmount = 0.5, techUnitId = UUID.nil
    , techIsInput = True, techIsReference = False
    , techActivityLinkId = UUID.nil, techProcessLinkId = Nothing, techLocation = loc }
...
spec = do
    ...
    describe "generateActivityUUIDFromActivity" $ do
        ...
        it "is deterministic for the same activity" $ do
            ...
            generateActivityUUIDFromActivity act `shouldBe`
        ...
        it "differs for different name" $ do
            ...
            generateActivityUUIDFromActivity a `shouldNotBe`
                generateActivityUUIDFromActivity b
    ...
    describe "buildSupplierIndex" $ do
        ...
        it "does not index input (non-reference) exchanges" $ do
            let act  = minimalActivity "consumer" "DE"
