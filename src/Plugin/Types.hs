...
import Types (Database, SimpleDatabase, Flow, Activity, ActivityMap, FlowDB, UnitDB)
...
data PluginBackend
    = Builtin                   -- ^ Haskell function, no process boundary
    | External !FilePath        -- ^ Standalone executable speaking JSON on stdin/stdout
    deriving (Show, Eq)
