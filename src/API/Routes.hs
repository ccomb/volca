...
type LCAAPI =
    "api"
        ...
        :> ( "db" :> Capture "dbName" Text :> "activity" :> Capture "processId" Text :> Get '[JSON] ActivityInfo
                ...
                :<|> "db" :> Capture "dbName" Text :> "activity" :> Capture "processId" Text :> "supply-chain" :> QueryParam "name" Text :> QueryParam "limit" Int :> QueryParam "min-quantity" Double :> QueryParam "offset" Int :> QueryParam "max-depth" Int :> QueryParam "location" Text :> QueryParam "classification" Text :> Get '[JSON] SupplyChainResponse
...
lcaServer dbManager maxTreeDepth password hostingConfig =
  ...
  where
    ...
    getActivitySupplyChain :: Text -> Text -> Maybe Text -> Maybe Int -> Maybe Double -> Maybe Int -> Maybe Int -> Maybe Text -> Maybe Text -> Handler SupplyChainResponse
    getActivitySupplyChain dbName processId nameFilter limitParam minQuantity offsetParam maxDepthParam locationFilter classificationFilter = do
        (db, sharedSolver) <- requireDatabaseByName dbManager dbName
        result <- liftIO $ Service.getSupplyChain db sharedSolver processId nameFilter limitParam minQuantity offsetParam maxDepthParam locationFilter classificationFilter
        case result of
            Left (Service.ActivityNotFound _) -> throwError err404{errBody = "Activity not found"}
            Left (Service.InvalidProcessId _) -> throwError err400{errBody = "Invalid ProcessId format"}
            Left (Service.MatrixError msg) -> throwError err500{errBody = BSL.fromStrict $ T.encodeUtf8 msg}
    ...
    postActivitySupplyChain :: Text -> Text -> Maybe Text -> Maybe Int -> Maybe Double -> Maybe Int -> Maybe Int -> Maybe Text -> Maybe Text -> SubstitutionRequest -> Handler SupplyChainResponse
    postActivitySupplyChain dbName processIdText nameFilter limitParam minQuantityParam offsetParam maxDepthParam locationFilter classificationFilter subReq = do
        (db, sharedSolver) <- requireDatabaseByName dbManager dbName
        (processId, _) <- resolveOrThrow db processIdText
        scalingResult <- liftIO $ Service.computeScalingVectorWithSubstitutions db sharedSolver processId (srSubstitutions subReq)
        case scalingResult of
            Left err -> throwServiceError err
            Right scalingVec -> do
                let result = Service.buildSupplyChainFromScalingVector db processId scalingVec nameFilter limitParam minQuantityParam offsetParam maxDepthParam locationFilter classificationFilter
                return result
