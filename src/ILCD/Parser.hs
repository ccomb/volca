...
parseILCDDirectory dir = do
    ...
    reportProgress Info $ printf "ILCD database loaded: %d activities, %d flows, %d units"
        (M.size $ sdbActivities fixedDb) (M.size $ sdbFlows fixedDb) (M.size $ sdbUnits fixedDb)
  ...
  where
    ...
    mkFlow uuid info =
        ...
        in Flow
            ...
            , flowSubcompartment = ilcdCompartment info >>= \(Compartment _ sc _) ->
    ...
    resolveUnit info = maybe UUID.nil id $
  ...
  where
    openTag s tag
        | isElement tag "exchange" =
            s { psInExchange = True, psTextAccum = []
              , psExInternalId = -1, psExFlowRef = ""
    ...
    buildProcess s = do
        ...
        if T.null (psBaseName s) then Nothing
        else Just ILCDProcessRaw
            { iprUUID = uuid
...
buildActivity flowInfoMap flowDB unitDB p = Activity
  ...
  where
    ...
    mkExchange refIdx raw =
        let flowUUID = ierFlowRef raw
            ...
            isElementary = maybe False (\fi -> ilcdFlowType fi == "Elementary flow")
        ...
        in if isElementary
            ...
            else TechnosphereExchange flowUUID (ierAmount raw) fUnitId isInput isRef
                    UUID.nil Nothing (ierLocation raw)
