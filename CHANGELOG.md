...
- The quality report's formula consistency check now lists an EcoSpold 2
  ...
  formulas as the dataset writes it and why it could not be read; a variable
  the file declares with two different amounts is named as such rather than as
  unknown. Same Info severity: the stored amounts are still the ones used. A
