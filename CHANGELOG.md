...
- `[[databases.patches]]`: declarative adjustments to the amounts a database
  states, the inventory-side sibling of `[[methods.patches]]`. A patch selects
  ...
  configuration keeps the load pure and reproducible, and says in the log how
  many rows each patch touched, where a script rewriting the source file says
  nothing. The patch list is part of what a database records having been built
  with, so a matrix cache written under another list is rebuilt instead of
  being read back with the amounts of the old one.
...
- A SimaPro export designates what an input buys from outside the file, rather
  than naming the product alone. The format has no column for the geography or
  the supplying activity: the convention puts both in the row's one name,
  `product {GLO}| activity | Cut-off, U`, and the writer wrote the bare
  product, which is the one thing that does not say which of the activities
  making that product the row bought from. A model of twenty-eight processes
  written to this format and read back beside the database it was cut from
  scored 28 % under the one it came from. A row whose maker travels in the same
  file keeps its bare name, the two rows naming each other by the product as
  they always have, and a product name that already designates is left as its
  source wrote it.
...
- An input designated the way a SimaPro export designates one finds its
  ...
  string as a product name. No database but the file it came from holds a name
  of that shape, so those inputs matched nothing at all, where the bare product
  name would have matched: a file naming `Urea {RER}| market for urea |` had
  its urea left unanswered beside a database supplying urea. A demand whose
  name matches nowhere is now asked again for what the designation states, the
  product plus the activity and the geography it names, which are the two
  things that tell twenty-seven activities making one product apart. A name
  that already matched still matches, and a refusal that had candidates, an
  incompatible unit or a geography the policy turns down, is still the answer.
...
- Whether a category is scored from each activity's location is asked of every
  ...
  *that* database holds, which is the whole answer only for as long as the
...
- A synonym that elementary flows of different names share no longer carries
  ...
  fossil in both climate change indicators. A flow is still found under its
  own name, and under a synonym no flow of another name lists, judged over a
  database and the databases it depends on together. Loading a database lists
  the synonyms it leaves out and the flows that share each. Against the
  EF v3.1 results an EcoSpold 2 release publishes, with the reference EF 3.1
  package, activities within 5 % go from 13 % to 78 % on cancer toxicity from
  inorganics and from 57 % to 88 % on fossil climate change.
