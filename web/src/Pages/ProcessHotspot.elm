module Pages.ProcessHotspot exposing (Model, Msg, page)
...
import Models.LCIA exposing (MethodSummary, ProcessHotspotResult)
import Models.Method exposing (MethodCollectionList, MethodCollectionStatus)
import Route exposing (ActivityTab(..), HotspotFlags, Route(..))
...
type alias Model =
    ...
    , result : RemoteData ProcessHotspotResult
    }
...
type Msg
    ...
    | SelectMethod String
    | ResultLoaded (Result Http.Error ProcessHotspotResult)
    | NavigateToProcess String
    | RequestLoadDatabase
    ...
    case msg of
        ...
        MethodsLoaded (Ok methods) ->
            ...
            case ( model.selectedMethod, List.any (\m -> Just m.msmId == model.selectedMethod) collMethods ) of
                ( Just mid, True ) ->
                    ...
                    , Effect.fromCmd (Api.loadProcessHotspot ResultLoaded model.dbName model.activityId (Maybe.withDefault "" model.selectedCollection) mid 20)
                    )
        ...
        SelectCollection name ->
            if name == "" then
                ...
                , Effect.fromShared (Shared.NavigateTo (ProcessHotspotRoute model.dbName model.activityId Nothing Nothing))
                )
                ...
                ( newModel
                , Effect.batch
                    [ Effect.fromShared (Shared.NavigateTo (ProcessHotspotRoute model.dbName model.activityId (Just name) Nothing))
                    , if List.isEmpty model.allMethods then
                        Effect.fromCmd (Api.loadMethods MethodsLoaded)
        ...
        SelectMethod methodId ->
            if methodId == "" then
                ...
                , Effect.fromShared (Shared.NavigateTo (ProcessHotspotRoute model.dbName model.activityId model.selectedCollection Nothing))
                )

            else
                ( { model | selectedMethod = Just methodId, result = Loading }
                , Effect.batch
                    [ Effect.fromCmd (Api.loadProcessHotspot ResultLoaded model.dbName model.activityId (Maybe.withDefault "" model.selectedCollection) methodId 20)
                    , Effect.fromShared (Shared.NavigateTo (ProcessHotspotRoute model.dbName model.activityId model.selectedCollection (Just methodId)))
                    ]
                )
        ...
        NavigateToProcess processId ->
            ( model
            , Effect.fromShared (Shared.NavigateTo (ActivityRoute Upstream model.dbName processId))
            )
...
view shared model =
    if not (Shared.isDatabaseLoaded shared model.dbName) then
        ...
        , body =
            div [ class "lcia-page" ]
                [ LCIAView.viewPageNavbar "Process Hotspot" activityInfo
                ...
                , HotspotView.viewProcessHotspotResult NavigateToProcess model.result
                ]
        }
