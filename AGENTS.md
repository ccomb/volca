...
- **No runtime crashes**: never use `error`, `throw`, `undefined`, or partial functions (`head`, `fromJust`). Functions that can fail must return `Either Text a` or `Maybe a`, never crash.
  In `src` and `app` this is checked rather than asked for: `hlint --hint=.hlint-partials.yaml src app` refuses the whole family, on Text, ByteString and Vector as well as on lists, and that file carries one baseline per site that stays with what makes that site safe. Fix the site or write the baseline - there is no third option. The test suite is deliberately outside it, for the reason `volca.cabal` gives beside its `-Wno-x-partial`: a fixture that does not match should crash the test at the line that destructured it.
