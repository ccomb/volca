...
- **No runtime crashes**: never use `error`, `throw`, `undefined`, or partial functions (`head`, `fromJust`). Functions that can fail must return `Either Text a` or `Maybe a`, never crash.
  In `src` and `app` this is checked rather than asked for: `hlint --hint=.hlint-partials.yaml --only="Avoid restricted function" src app` refuses the element accessors, `read`, the unseeded folds, `error`, `fromJust`, `undefined` and `unsafePerformIO`, on Text, ByteString and Vector as well as on lists, and that file carries one baseline per site that stays with what makes that site safe. It does not reach the indexing operators (`V.!`, `M.!`) or the strict `decodeUtf8`, which crash the same way and are named in that file as a job of their own. Fix the site or write the baseline - there is no third option. The test suite is deliberately outside it, for the reason `volca.cabal` gives beside its `-Wno-x-partial`: a fixture that does not match should crash the test at the line that destructured it.
...
- **A factor is read where the data says, never where it seems close.** How a flow meets a characterization factor is written down, not decided in code: a flow reads a factor at its own subcompartment, at the line its substance writes for the whole medium, or where an `if_absent` row of `data/compartments.csv` sends it, and nowhere else (README, "How a flow meets a factor's subcompartment"). A new correspondence between two vocabularies is a row in that file, which method and database authors can read and change; a rule in `Method.Mapping` that lends one subcompartment's factor to another is the hidden behaviour this forbids.
- **An index asserts that its key determines its value.** `fromList` silences duplicate keys, so a key that is only part of a wider identity (an activity UUID taken from the (activity, product) pair, a normalised name taken from a name) quietly keeps one row and drops the rest. Either the key really is the primary key, or the honest type is `Map k (NonEmpty v)` and every reader has to say what it does with several.
  In the format parsers and the tables the engine ships, `hlint` enforces this: `Data.Map.fromList` is an error there, and `.hlint.yaml` carries one baseline per site where the key really does determine its value, with what makes that true. Fix the site or add the baseline with its reason - there is no third option. The baseline names a function rather than a line, so moving the site out of that function fires the error again; nothing removes a baseline whose site has gone, which is a job for whoever reads the list next.
